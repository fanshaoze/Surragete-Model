...
def train_epoch(model, training_data, optimizer, device, smoothing=False, criterion=None, likelihood=None, gp=None):
...
#        tf_mse += F.mse_loss(final,eff).cpu().item() * path.size(0)
        ...
        # backward
        loss = -criterion(pred, eff.squeeze(1))
...
def train(model, training_data, validation_data, optimizer, args, vocab, list_of_refs_dev, validation_data_combined, criterion, likelihood, gp):
    ''' Start training '''
    ...
    for epoch_i in range(args.epoch):
        ...
        train_loss, train_accu = train_epoch(
            model, training_data, optimizer, args.device, criterion=criterion,likelihood=likelihood, gp=gp)

        print('  - (Training)   mse: {mse: 8.5f}, accuracy: {accu:3.3f} %, '\
              'elapse: {elapse:3.3f} min'.format(
                  mse=train_loss, accu=100*train_accu,
        ...
        print('  - (Validation) mse: {mse: 8.5f}, accuracy: {accu:3.3f} %, '\
                'elapse: {elapse:3.3f} min'.format(
                    mse=valid_loss, accu=100*valid_accu,
                    elapse=(time.time()-start)/60))
        ...
        if epoch_i != 0 and epoch_i % args.bleu_valid_every_n == 0:
            ...
            early_stopping_with_saving(valid_loss, model, epoch_i, gp, likelihood)
...
def main():
    ...
    if args.data_train:
        ...
        transformer = get_model(args, load_weights=args.load_weights)
        ...
        train( transformer, data_loader_training, data_loader_dev, optimizer ,args, vocab, list_of_refs_dev, data_loader_bleu, criterion, likelihood, gp)
